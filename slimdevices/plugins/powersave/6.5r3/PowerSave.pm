	...
	foreach my $client (Slim::Player::Client::clients()) {
		...
		if ((Slim::Buttons::Common::mode ($client) !~ /^OFF/)
			...
			if (checkPlaymode ($client)) {
				...
				$::d_plugins && msg("PowerSave check: client eligible for sleep: powersave-time=${time}\n");
				# reset timer after wakeup
				if ($powerSaveTimers{$client}{psactivated} == 1) {
					$powerSaveTimers{$client}{time} = time;
					$powerSaveTimers{$client}{psactivated} = 0;
					$::d_plugins && msg("PowerSave nosleep: reset timer after wakeup\n");
				} elsif ($powerSaveTimers{$client}{lastirtime} != int ($client->lastirtime)) {
					$powerSaveTimers{$client}{lastirtime} = int ($client->lastirtime);
					$powerSaveTimers{$client}{time} = time;
					$::d_plugins && msg("PowerSave nosleep: ir activity: lastirtime=${powerSaveTimers{$client}{lastirtime}}\n");
				} elsif ($powerSaveTimers{$client}{modechgtime} == 0) {
					$powerSaveTimers{$client}{modechgtime} = time;
					$::d_plugins && msg("PowerSave nosleep: mode change: modechgtime=${powerSaveTimers{$client}{modechgtime}}\n");
					...
					$::d_plugins && msg("PowerSave sleep at @{[int(time)]} : time=${powerSaveTimers{$client}{time}} modechgtime=${powerSaveTimers{$client}{modechgtime}}\n");
				}
			} else {
				$powerSaveTimers{$client}{modechgtime} = 0;
			}
