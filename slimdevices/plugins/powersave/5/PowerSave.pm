...
sub getDisplayName() {
	return string('PLUGIN_POWERSAVE');
}
	...
	'right' => sub  {
		...
		if ($browseMenuChoices[$menuSelection{$client}] eq string('PLUGIN_POWERSAVE_OFF')) {
			...
			Slim::Display::Animation::showBriefly
				($client, string('PLUGIN_POWERSAVE_TURNING_ON'), '');
			...
			$browseMenuChoices[$menuSelection{$client}] =
				string('PLUGIN_POWERSAVE_OFF');
			Slim::Display::Animation::showBriefly
				($client,string('PLUGIN_POWERSAVE_TURNING_OFF'),'');
...
sub checkPowerSaveTimer {
	...
	foreach my $client (Slim::Player::Client::clients()) {
		...
		unless (exists $powerSaveTimers{$client}) {
			...
			$powerSaveTimers{$client}{lastirtime} =
				int ($client->lastirtime);
